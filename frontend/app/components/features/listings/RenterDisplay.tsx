...
import { createSession, sendMessage, getBookingsByStorageLocationId } from "@/lib/api";
    ...
    try {
      ...
      if (!sid) {
        ...
        if (
          ...
          throw new Error(sessionRes.data.message || "Failed to create chat session.");
        }
      }

      const messageRes = await sendMessage(sid, newMessage);
      if (messageRes.status >= 200 && messageRes.status < 300 && messageRes.data.data) {
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      ...
      <TabsContent value="details" className="mt-4">
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">{listing.name}</h2> {/* Listing Title */}
          <p className="text-gray-600">{listing.description}</p>
          <div className="grid grid-cols-2 gap-4 pt-4">
            <div>
              <h4 className="font-semibold text-lg mb-1">Dimensions</h4>
              <p>{listing.storageLength && listing.storageWidth && listing.storageHeight
      ...
      <TabsContent value="messages" className="mt-4 flex flex-col h-[60vh]">
        <div className="flex-1 overflow-y-auto border rounded p-4 bg-gray-50">
          {messages.length === 0 ? (
            <p className="text-gray-400">Start a conversation with the lender!</p>
          ) : (
